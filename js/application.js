...
d3.json(`data/${pdbid}.json`, (data) => {
  ...
  //heatmap();
  var matrix = [[]];
  H = barchartData.sizes[0].length + 1;
  for(var i = 1; i < H; i++){
      matrix.push([]);
  }
  W = 2;
  ...
  matrix[0][0] = "Sizes";
  matrix[1][0] = barchartData.mutation[0];

  // populate matrix with WildType
  for (var x = 0; x < W; x++) {
      for (var y = 1; y < H; y++) {
          if (x == 0) {
              matrix[x][y] = barchartData.sizes[0][y-1];
          } else if (x == 1) {
              matrix[x][y] = barchartData.counts[0][y-1];
          }
      }
  }
...
//   }
//
//   const x = d3.scaleBand().bandwidth([0, width]);
//   const y = d3.scaleBand().bandwidth([0, height]);
//
//   const z = d3.scaleLinear().range([0, 1]).clamp(true);
//   const c = d3.scaleLinear().range([unstable_color, middle_color, stable_color]);
//
//   //options nemu
